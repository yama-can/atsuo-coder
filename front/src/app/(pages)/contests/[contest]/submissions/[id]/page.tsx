...
export default async function Home(params: { params: { [key: string]: string } }) {
	...
	let allowOtherUser = true;

	if (contest[0].start + contest[0].period > Date.now()) {
		allowOtherUser = false;
	}

	const result = await sql.query(`SELECT sourceCode, judge, task from submissions WHERE id = ? ${allowOtherUser ? "" : "AND user = ?"}`, [params.params.id, user?.id]);
